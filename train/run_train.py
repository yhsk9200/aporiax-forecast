...
Prometheus에서 시계열을 당겨(forecaster.prom) 특징을 만들고(forecaster.features)
gradient-boosting 회귀를 학습, persistence 베이스라인과 비교한 뒤 model version을
MLflow 레지스트리에 metric과 함께 등록한다. 특징 로직은 서빙과 공유한다(skew 방지).
...
from forecaster.features import FEATURE_COLS, build_training_frame
